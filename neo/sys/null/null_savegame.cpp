...
========================
*/
idLocalUserNULL * GetLocalUserFromSaveParms( const saveGameThreadArgs_t & data ) {
	if( ( data.saveLoadParms != NULL ) && ( data.saveLoadParms->inputDeviceId >= 0 ) ) {
		...
		if( user != NULL ) {
			idLocalUserNULL * userWin = static_cast< idLocalUserNULL * >( user );
...
int idSaveGameThread::Save() {
	idLocalUserNULL * user = GetLocalUserFromSaveParms( data );
	if( user == NULL ) {
	...
	for( int i = 0; i < callback->files.Num() && ret == ERROR_SUCCESS && !callback->cancelled; i++ ) {
		...
		if( outputFile == NULL ) {
			idLib::Warning( "[%s]: Couldn't open file for writing, %s. Error = %s", __FUNCTION__, tempFileName.c_str(), savegameError() );
			file->error = true;
		...
		if( ( file->type & SAVEGAMEFILE_PIPELINED ) != 0 ) {
			...
			while( inputFile->NextWriteBlock( & block ) ) {
				if( ( size_t )outputFile->Write( block.data, block.bytes ) != block.bytes ) {
					idLib::Warning( "[%s]: Write failed. Error = %s", __FUNCTION__, savegameError() );
					file->error = true;
					callback->errorCode = SAVEGAME_E_INSUFFICIENT_ROOM;
					ret = -1;
					break;
...
int idSaveGameThread::Enumerate()
{
	...
	if( fileSystem->IsFolder( saveFolder, "fs_savePath" ) == FOLDER_YES ) {
		...
		for( int i = 0; i < fileList.Num() && !callback->cancelled; i++ ) {
			...
			// JDC: I hit this all the time
			//assert( fileSystem->IsFolder( directory.c_str(), "fs_savePath" ) == FOLDER_YES );
		}
		fileSystem->FreeFileList( files );
	} else {
		callback->errorCode = SAVEGAME_E_FOLDER_NOT_FOUND;
